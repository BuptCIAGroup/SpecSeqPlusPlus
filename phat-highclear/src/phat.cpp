...
void print_help() {
    ...
    std::cerr << "--highclear   --  use highclear approach" << std::endl;
}
...
void parse_command_line(int argc, char** argv, bool& use_binary, Representation_type& representation, Algorithm_type& algorithm,
    ...
    for (int idx = 1; idx < argc - 2; idx++) {
        ...
        else if (option == "--highclear")highclear = true;
...
void compute_pairing(std::string input_filename, std::string output_filename, bool use_binary, bool verbose, bool dualize, bool highclear) {
    ...
    //MODIFY BY BUPT BEGIN
    if (highclear) {
        double highclear_timer = omp_get_wtime();
        LOG("MaxDimClearing ...")
            bupt::MAXDIM_HIGHCLEAR(matrix);
        double highclear_time = omp_get_wtime() - highclear_timer;
        double highclear_time_rounded = floor(highclear_time * 10.0 + 0.5) / 10.0;
        LOG("MaxDimClear took " << std::setiosflags(std::ios::fixed) << std::setiosflags(std::ios::showpoint) << std::setprecision(1) << highclear_time_rounded << "s")
    }
    //MODIFY BY BUPT END
    if (dualize) {
    ...
    LOG("Computing persistence pairs took " << std::setiosflags(std::ios::fixed) << std::setiosflags(std::ios::showpoint) << std::setprecision(5) << pairs_time << "s")
...
#define COMPUTE_PAIRING(Representation) \
    switch( algorithm ) { \
    case STANDARD: compute_pairing< phat::Representation, phat::standard_reduction> ( input_filename, output_filename, use_binary, verbose, dualize, highclear ); break; \
    case TWIST: compute_pairing< phat::Representation, phat::twist_reduction> ( input_filename, output_filename, use_binary, verbose, dualize, highclear ); break; \
    case ROW: compute_pairing< phat::Representation, phat::row_reduction >( input_filename, output_filename, use_binary, verbose, dualize, highclear ); break; \
    case SPECTRAL_SEQUENCE: compute_pairing< phat::Representation, phat::spectral_sequence_reduction >( input_filename, output_filename, use_binary, verbose, dualize,highclear ); break; \
    case CHUNK: compute_pairing< phat::Representation, phat::chunk_reduction >( input_filename, output_filename, use_binary, verbose, dualize, highclear ); break; \
    case SWAP: compute_pairing< phat::Representation, phat::swap_twist_reduction> ( input_filename, output_filename, use_binary, verbose, dualize, highclear ); break; \
    case EXHAUSTIVE: compute_pairing< phat::Representation, phat::exhaustive_compress_reduction> ( input_filename, output_filename, use_binary, verbose, dualize, highclear ); break; \
    case RETROSPECTIVE: compute_pairing< phat::Representation, phat::lazy_retrospective_reduction> ( input_filename, output_filename, use_binary, verbose, dualize, highclear ); break; \
    case CHUNK_SEQUENTIAL: int num_threads = omp_get_max_threads();	\
                           omp_set_num_threads( 1 ); \
                           compute_pairing< phat::Representation, phat::chunk_reduction >( input_filename, output_filename, use_binary, verbose, dualize, highclear ); \
...
int main(int argc, char** argv) {
    ...
    //MODIFY BY BUPT
    bool highclear = false;
    parse_command_line(argc, argv, use_binary, representation, algorithm, input_filename, output_filename, verbose, dualize, highclear);
    //MODIFY BY BUPT
